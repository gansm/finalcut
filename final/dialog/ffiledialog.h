...
class FFileDialog : public FDialog
{
  ...
  private:
    ...
    void sortDir();
    auto readDir() -> int;
    void getEntry (const std::string&, std::string&& name, const struct dirent*);
    void followSymLink (const std::string&, FDirEntry&) const;
    ...
    auto isRootDirectory (const std::string&) const -> bool;
